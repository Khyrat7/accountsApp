...
export default LoginScreen = props => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      title: Platform.OS === 'android' ? null : login ? 'Login' : 'Sign up',
      headerRight: () => (
        <TouchableOpacity
          onPress={() => {
            if (login) {
  ...
  // text Validation
  emailVaildation = () => {
    const isValid = LoginValidation.isValidEmail(email);
  ...
  passwordValidation = () => {
    const isValid = LoginValidation.isValidPassword(password);
    if (!isValid) {
